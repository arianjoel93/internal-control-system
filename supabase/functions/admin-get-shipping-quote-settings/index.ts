...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await adminClient
      ...
      .select('id, provider, fedex_base_url, fedex_origin_postal_code, fedex_client_id, fedex_client_secret, fedex_account_number, fedex_child_key, fedex_child_secret, is_active, created_at, updated_at')
      .eq('provider', 'fedex')
      .maybeSingle();
    ...
    const message = error instanceof Error ? error.message : 'No se pudo consultar la configuración del cotizador.';
